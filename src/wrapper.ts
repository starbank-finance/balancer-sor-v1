...
export class SOR {
  ...
  async fetchPools(): Promise<boolean> {
    try {
      let allPools = await this.pools.getAllPublicSwapPools(this.poolsUrl);
  ...
  async getSwaps(
    ...
    if (this.isAllFetched) {
      ...
      [swaps, total, marketSp, totalConsideringFees] = await this.processSwaps(
        TokenIn,
        TokenOut,
        SwapType,
        SwapAmt,
  ...
  async fetchFilteredPairPools(
    ...
    try {
      ...
      allSwaps.forEach(swap => {
        swap.forEach(seq => {
          seq.forEach(p => {
            if (!filteredPools.includes(p.pool)) filteredPools.push(p.pool);
          });
        });
      });
